...
zoomCodes = ['771 9385 9390', '771 9385 9001', '75880221688']
    ...
    def on_status(self, status):
        ...
        if not 'RT @' in status.text:
            ...
            for code in zoomCodes:
                if code in status.text:
                    print('FOUND ZOOM CODE: ' + str(code) + '\n' + 'FROM USER: ' + str(status.user.url))
                    try:
...
def start_mining(queries):
    ...
    consumer_key = Secret.TWITTER_APP_KEY
    consumer_secret = Secret.TWITTER_APP_KEY_SECRET
    access_token = Secret.TTWITTER_ACCESS_TOKEN
...
start_mining(['#zoomcodes', '#zoomcode', '#zoombombing', '#zoomclasscodes'])